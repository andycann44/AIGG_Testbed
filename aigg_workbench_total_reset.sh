...
if [[ -f "$F" ]]; then
  BK="Assets/AIGG/_Backups/TotalReset_$(date +%Y%m%d_%H%M%S)"
  mkdir -p "$BK"
...
namespace Aim2Pro.AIGG.Workbench
{
  internal class CurveSpec {
    public string side; public float degrees; public float? rows;
    public CurveSpec(string side, float degrees, float? rows) { this.side=side; this.degrees=degrees; this.rows=rows; }
  }
  ...
  public class WorkbenchWindow : EditorWindow
  {
    ...
    // Words we always ignore as tokens (do NOT include semantic ones like "under", "sloping", etc)
    ...
    // Allow as filler
    static readonly HashSet<string> AllowedFillers = new HashSet<string>{ "with","and","then","a","the","track","tracks" };
    ...
    void OnGUI()
    {
      ...
      EditorGUILayout.HelpBox("If parsing fails OR leaves unmatched/unaccounted tokens, output stays EMPTY.", MessageType.Info);
    }
    ...
    void ParseLocal()
    {
      try
      {
        ...
        // Gate 2: residual semantics (even if in lexicon) → EMPTY
    ...
    static string[] GetResidualMeaningfulTokens(string norm)
    {
      ...
      foreach (Match m in Regex.Matches(norm,
      ...
      // Force certain semantics to remain residual until implemented
      var toksAll = toks;
      ...
      foreach (var ct in critical) if (tokset.Contains(ct)) covered.Remove(ct); // ensure not covered
      ...
      // Make sure critical tokens are flagged as residual if present
      foreach (var ct in critical) if (tokset.Contains(ct) && !residual.Contains(ct)) residual.Add(ct);

      return residual.Distinct().ToArray();
    }

    static string BuildSimpleTrackJson(float length, float width,
      float? missingChance = null, float? gapChance = null, List<CurveSpec> curves = null)
    {
      var sb = new StringBuilder();
      sb.Append("{\n");
      sb.Append("  \"track\": {\n");
    ...
    static string TryConvertNLToCanonical(string nl)
    {
        ...
        RegexOptions.Multiline))
      {
        ...
        float? rows = m.Groups[3].Success ? float.Parse(m.Groups[3].Value, CultureInfo.InvariantCulture) : (float?)null;
      ...
      foreach (var t in asms.SelectMany(a => { try { return a.GetTypes(); } catch { return Array.Empty<Type>(); } }))
      {
        ...
        foreach (var m in t.GetMethods(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Static))
        {
          ...
          try
          {
            ...
            if (LooksLikeFallback(s)) { _diag = $"Suppressed fallback from {t.FullName}.{m.Name}\n" + _diag; continue; }
...
touch "$F"
echo "Replaced WorkbenchWindow.cs with a compile-clean, strict, OpenAI-wired version."
