...
namespace Aim2Pro.AIGG
{
    ...
    // Diagnostics show matched/unmatched tokens grouped by source (intents/macros/lexicon/commands).
    public class WorkbenchWindow : EditorWindow
    {
        ...
        void OnGUI()
        {
            ...
            EditorGUILayout.HelpBox(
                ...
                "Diagnostics show which SPEC files include your tokens.\n" +
        ...
        void ParseLocal()
        {
            ...
            // 1) Try NLToJson.GenerateFromPrompt
        ...
        static List<string> Tokenize(string s)
        {
            ...
            // keep it simple to avoid escaping issues
            var seps = new char[] { ' ', ',', ';', '.', ':', '(', ')', '[', ']', '{', '}', '/', '-', '+' };
        ...
        static Dictionary<string, HashSet<string>> LoadSpecSetsBySource()
        {
            ...
            if (map["intents"].Count == 0 &&
                map["macros"].Count == 0 &&
                map["lexicon"].Count == 0 &&
                map["commands"].Count == 0)
            {
        ...
        static void AddWordsFromJson(string assetPath, HashSet<string> set)
        {
            ...
            foreach (var w in parts) set.Add(w);
        }
        ...
        static void CopySkeletonIntent(string phrase)
        {
            ...
            if (phrase.Length == 0)
            {
                Debug.LogWarning("[AIGG] Click a chip or enter an unmatched phrase first.");
                return;
            }

            // build JSON without interpolated verbatim strings (avoid brace escaping issues)
