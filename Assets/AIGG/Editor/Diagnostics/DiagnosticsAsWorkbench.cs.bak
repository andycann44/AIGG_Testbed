...
{
  // Open either way:
  //   Window → Aim2Pro → Aigg → Workbench (Diagnostics Alias)
  //   Window → Aim2Pro → Aigg → Diagnostics (Full)
  public class DiagnosticsAsWorkbench : EditorWindow
  {
    ...
    private void OnGUI()
    {
      ...
      {
        if (GUILayout.Button("Parse NL (local)", GUILayout.Height(26))) RunLocal();
        if (GUILayout.Button("Scan (Matched / Unmatched)", GUILayout.Height(26))) ScanNow();
    ...
    {
      // Prefer real engine: Aim2Pro.AIGG.AIGG_LocalIntentEngine.RunToJson(string)
      try {
        ...
        // Fallback: build a simple envelope so the pipeline stays alive
