 ...
 * Now wired to Assets/AIGG/Spec (not Resources).
 * Adds: OpenWithJson(string), Spec Health (reads Assets/AIGG/Spec/spec_index.json).
 */
...
namespace Aim2Pro.AIGG
{
  ...
  private string _specFolder = "Assets/AIGG/Spec";
        ...
        // Router shim: allow PreMergeRouterAPI.Route(json) -> OpenWithJson(json)
        ...
        // Backward compatible external entry
        ...
        Vector2 svPaste, svDetail, svPreview, svHealth;

        // >>> IMPORTANT: new Spec root <<<
        static string SpecRoot => "Assets/AIGG/Spec";

        // spec_index.json (defines required files; fallback to classic 7)
        static string SpecIndexPath => Path.Combine(SpecRoot, "spec_index.json");
        static readonly string[] DefaultRequiredFiles = new[]{
            "lexicon.json","fieldmap.json","intents.json","schema.json",
            "registry.json","commands.json","macros.json"
        };
        ...
        void OnEnable() { Directory.CreateDirectory(SpecRoot); }
        ...
        // -------- Spec Health --------
        struct HealthItem { public string name; public bool exists; }
        List<HealthItem> _health = new List<HealthItem>();
        string _healthSummary = "";

        string[] LoadRequiredFiles()
        {
            try {
                if (File.Exists(SpecIndexPath))
                {
                    object obj;
                    if (MiniJSON.TryDeserialize(File.ReadAllText(SpecIndexPath, Encoding.UTF8), out obj))
                    {
                        var dict = obj as Dictionary<string,object>;
                        if (dict != null && dict.TryGetValue("files", out var arrObj) && arrObj is List<object> arr)
                        {
        ...
        void CheckSpecHealth(bool autoCreateMissing)
        {
            Directory.CreateDirectory(SpecRoot);
            _health.Clear();
            var need = LoadRequiredFiles();
            int missing = 0;
            foreach (var name in need)
            {
                var p = Path.Combine(SpecRoot, name);
                bool ok = File.Exists(p);
                if (!ok && autoCreateMissing) { EnsureFileForName(p, name); ok = File.Exists(p); }
                _health.Add(new HealthItem{ name = name, exists = ok });
                if (!ok) missing++;
            }
            _healthSummary = missing == 0 ? "All required spec files present." : (missing.ToString() + " missing. Click 'Create Missing'.");
        }

        void EnsureFileForName(string path, string fileNameHint=null)
        {
            Directory.CreateDirectory(Path.GetDirectoryName(path));
            if (!File.Exists(path))
            {
                var seed = new Dictionary<string, object>();
                var fn = (fileNameHint ?? Path.GetFileName(path)).ToLowerInvariant();
                if (fn.Contains("lexicon"))   seed["synonyms"]      = new Dictionary<string,object>();
                else if (fn.Contains("fieldmap"))  seed["englishToPath"] = new Dictionary<string,object>();
                else if (fn.Contains("intents"))   seed["intents"]       = new List<object>();
                else if (fn.Contains("schema"))    seed["version"]       = "v1";
                else if (fn.Contains("registry"))  seed["components"]    = new List<object>();
                else if (fn.Contains("commands"))  seed["commands"]      = new List<object>();
                else if (fn.Contains("macros"))    seed["macros"]        = new List<object>();
                else seed["note"] = "seeded by Paste & Merge";
        ...
        void OnGUI()
        {
            ...
            {
                EditorGUILayout.LabelField("Spec Folder", SpecRoot);
                using (new EditorGUILayout.HorizontalScope())
                {
                    if (GUILayout.Button("Reveal Spec Folder", GUILayout.Width(160))) EditorUtility.RevealInFinder(SpecRoot);
                    if (GUILayout.Button("Check Spec Health", GUILayout.Width(160))) { CheckSpecHealth(false); }
                    if (GUILayout.Button("Create Missing", GUILayout.Width(140))) { CheckSpecHealth(true); }
                }
                if (!string.IsNullOrEmpty(_healthSummary)) EditorGUILayout.HelpBox(_healthSummary, MessageType.Info);
                if (_health.Count > 0)
                {
                    svHealth = EditorGUILayout.BeginScrollView(svHealth, GUILayout.MaxHeight(120));
                    foreach (var h in _health)
                    {
                        var ok = h.exists ? "OK" : "MISSING";
                        EditorGUILayout.LabelField("• " + h.name + " — " + ok);
                    }
            ...
            EditorGUILayout.Space();
            EditorGUILayout.HelpBox("Tip: paste JSON or simple \"alias => canonical\" lines for Lexicon/Fieldmap.", MessageType.None);
        }
        ...
        void Analyze()
        {
            ...
            EnsureFileForName(path, Path.GetFileName(path));
            ...
            {
                status = "Error: Target JSON failed to parse.";
                errCount++;
                return;
            }
            ...
            try
            {
                switch (target)
                {
                    ...
                    case TargetType.Registry:
                        ...
                        status = (target.ToString() + " ready to replace.");
                        break;
                }
            }
            catch (Exception ex)
        ...
        bool Apply()
        {
            ...
            switch (target)
            {
                ...
                case TargetType.Schema:
                case TargetType.Registry:  result = incomingObj; break;
            }
  ...
  private void EnsureTargetFileExists(string path) {
    ...
    static class MiniJSON
    {
        ...
        sealed class Serializer
        {
            ...
            Serializer(bool pretty) { this.pretty = pretty; builder = new StringBuilder(); }
