...
namespace Aim2Pro.AIGG {
  [Serializable] public class ProposedFix {
    ...
    public System.Collections.Generic.Dictionary<string, string> files = new System.Collections.Generic.Dictionary<string, string>(); // optional
  }
  ...
  public static class AIAutoFix {
    ...
    public static ProposedFix Ask(string apiKey, string model, string nl, string normalized, string canonical, List<string> unmatched, List<string> missingCmds, out string error) {
      ...
      var sys = "You update a game parsing spec. Return ONLY a JSON object with keys: \"commands\":[string], \"macros\":[string], \"fieldMap\":[{\"key\":\"<phrase>\",\"value\":\"<canonical.path>\"}], optional \"canonical\":\"<strict canonical JSON>\", and optional \"files\":{...}. For \"files\", each value MUST be a single STRING containing the EXACT file contents. Allowed file names: aliases.json, commands.json, defaults.json, fieldmap.json, generators.json, intents.json, lexicon.json, macros.json (or macros.txt), patterns.json, presets.json, registry.json, rules.json, schema.json. macros.* should be newline-separated tokens; fieldmap.* should be lines of \"key => value\". Only include files you are confident about. Return only the JSON object.";
