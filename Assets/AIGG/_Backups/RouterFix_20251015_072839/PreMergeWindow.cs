...
namespace Aim2Pro.AIGG.PreMerge
{
  ...
  {
    [MenuItem(Aim2Pro.AIGG.MenuSpec.Legacy + "/Pre-Merge (NL)")]
    ...
    string _missing = "";
    string _canonical = "{ }";
    bool   _blockPreview = false;   // hide preview + disable Send when true
    ...
    void RunPreMerge(bool localOnly=false) {
      ...
      if (useAI && aiFirst && !localOnly) { AskAI(); return; }
      ...
      // HARD VALIDATION (guarantee hiding + explicit curve message)
      var hardMiss = HardValidate(_nl, _canonical);
      var allMiss  = new List<string>();
      if (res.missing != null) allMiss.AddRange(res.missing);
      allMiss.AddRange(hardMiss);
      _missing      = JoinBullets(Dedup(allMiss));
      _blockPreview = !string.IsNullOrEmpty(_missing);
    }
    ...
    void AskAI() {
      ...
      try {
        ...
        if (!string.IsNullOrEmpty(json)) {
          ...
          // Re-validate: hide/disable if missing
          var postMissing = HardValidate(_nl, _canonical);
    ...
    // ---------- Hard validation (always run before showing/allowing Send) ----------
    List<string> HardValidate(string nl, string canonicalJson) {
      var miss = new List<string>();

      // Parse canonical
      if (string.IsNullOrWhiteSpace(canonicalJson)) { miss.Add("no canonical json"); return miss; }
      object root;
      if (!MiniJSON.TryDeserialize(canonicalJson, out root) || !(root is Dictionary<string,object> d)) {
        miss.Add("canonical structure (expected object)"); return miss;
      }

      // Helpers
      bool HasPath(Dictionary<string,object> obj, string[] path) {
        object cur = obj;
        foreach (var seg in path) {
          if (cur is Dictionary<string,object> dd) {
            if (!dd.TryGetValue(seg, out cur)) return false;
          } else return false;
        }
        return cur != null;
      }
      bool MentionsCurve(string s) {
        return Regex.IsMatch(s, "\\b(left|right)\\b.*\\b(\\d+)\\s*(?:deg|degree|degrees)\\b.*\\b(\\d+)\\s*rows?\\b", RegexOptions.IgnoreCase)
            || Regex.IsMatch(s, "\\b(\\d+)\\s*(?:deg|degree|degrees)\\b.*\\b(left|right)\\b.*\\b(\\d+)\\s*rows?\\b", RegexOptions.IgnoreCase);
      }

      // Always require dims
      if (!HasPath(d, new[]{"track","length"})) miss.Add("$.track.length");
      if (!HasPath(d, new[]{"track","width"}))  miss.Add("$.track.width");

      // Curve expectation
      if (MentionsCurve(nl)) {
        bool hasPlan = d.TryGetValue("plan", out var p) && p is List<object> arr && arr.Count>0;
        bool hasCurve=false, okSide=false, okDeg=false, okRows=false;
        if (hasPlan) {
          foreach (var o in (List<object>)p) {
            var item = o as Dictionary<string,object>; if (item==null) continue;
            if (item.TryGetValue("action", out var act)
                && string.Equals(act?.ToString(),"curveRowsOver",StringComparison.OrdinalIgnoreCase)) {
              hasCurve = true;
              if (item.TryGetValue("args", out var args) && args is Dictionary<string,object> ad) {
                okSide = ad.ContainsKey("side");
                okDeg  = ad.ContainsKey("deg");
                okRows = ad.ContainsKey("rows");
              }
              break;
            }
          }
        }
        if (!hasCurve || !(okSide && okDeg && okRows))
          miss.Add("curve step (plan[].action='curveRowsOver')");
      }
      return miss;
    }
    ...
    List<string> Dedup(List<string> xs){
      var set=new HashSet<string>(xs ?? new List<string>());
      return new List<string>(set);
    }

    // split/save AI envelope & optionally route first file
