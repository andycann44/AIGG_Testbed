...
namespace Aim2Pro.AIGG {
  public static class PreMergeRouterAPI {
    ...
    // Legacy overload (keeps old callers compiling; still strict)
    public static void Route(params string[] args) {
      if (args == null || args.Length == 0) { Fail("Legacy Route() with no args.", "", null, null); return; }
      if (args.Length == 1) { var s0=args[0]??""; if (LooksLikeJsonObject(s0)) StrictRoute("", s0, ""); else Fail("Legacy Route(arg) not understood.", s0, null, null); return; }
      if (args.Length == 2) {
        ...
        Fail("Legacy Route(a,b): need nl, canonical, diagnostics.", a, bIsCanonical?b:null, bIsDiag?b:null); return;
      }
    ...
    private static void StrictRoute(string nl, string canonicalJson, string diagnosticsJson) {
      ...
      foreach (var a in asm) {
        ...
        if (m != null) { m.Invoke(null, new object[] { canonicalJson }); return; }
      }
      EditorGUIUtility.systemCopyBuffer = canonicalJson;
    ...
    private static bool LooksLikeJsonObject(string s) { if (string.IsNullOrWhiteSpace(s)) return false; s = s.TrimStart(); return s.Length > 0 && s[0] == '{'; }
