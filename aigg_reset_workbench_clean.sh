...
if [[ -f "$F" ]]; then
  BK="Assets/AIGG/_Backups/CleanReset_$(date +%Y%m%d_%H%M%S)"
  mkdir -p "$BK"
...
{
  // No tuples: small POCO for curves
  ...
  public class WorkbenchWindow : EditorWindow
  {
    ...
    // Tokens ignored in "unmatched" (so they don't block JSON); numbers are always ignored
    static readonly HashSet<string> IgnorableTokens = new HashSet<string> {
      ...
      "degree","degrees","deg",
      "over","under","across","through","around","between","along","then","from","going","into","onto","via"
    };

    // ===== self-heal prefs + script paths =====
    const string PrefAutoPatch = "AIGG.Workbench.AutoPatchLexicon";
    bool _autoPatch = true;     // read in OnEnable
    ...
    void OnEnable()
    {
      _autoPatch = EditorPrefs.GetBool(PrefAutoPatch, true);
    }
    ...
    void OnGUI()
    {
      ...
      EditorGUILayout.HelpBox(
        "Both tools read 7 Spec files in Assets/AIGG/Spec. If parsing fails, returns a scenePlan, OR leaves unmatched tokens, output stays EMPTY.",
        MessageType.Info);
    }
    ...
    void ParseLocal()
    {
      try
      {
        ...
        // record for self-heal
        _lastUnmatched = unmatched.ToList();

        // 1) self-heal lexicon, then auto-reparse
        if (_autoPatch && unmatched.Length > 0 && !_isReparsing)
        {
          ...
          if (added > 0)
          {
            ...
            ShowNotification(new GUIContent($"Lexicon updated (+{added}). Re-parsing…"));
            return;
          }
        }
        ...
        // 2) STRICT: if any unmatched remain, do NOT produce JSON
        if (unmatched.Length > 0)
        {
          _json = "";
          extraNote = "Output cleared: unmatched tokens present. Use Self-heal or add to Spec.";
          ShowNotification(new GUIContent("Unmatched tokens → output EMPTY"));
        }
        else
        {
          // 3) When clean, build JSON (canonical) or try interpreters
    ...
    // Build JSON (supports multi-curves with optional rows)
    ...
    // NL → canonical with no-space + multi-curves + "over N rows"
    static string TryConvertNLToCanonical(string nl)
    {
      if (string.IsNullOrWhiteSpace(nl)) return null;

      // lower + insert virtual spaces at alpha↔digit boundaries (handles "right35")
      ...
      // curves/chicanes (MULTI) – "left 45 degree curve" OR "45 degree left chicane" and no-space "right35"
    ...
    {
      // Try as TextAsset first (no extension, .txt, or .json)
      string text = null;
      ...
      // Fallback: read from disk if not imported as TextAsset
      if (text == null) {
    ...
    // ----- Self-heal (local) wired to Unmatched -----
...
touch "$F"
echo "Rewritten cleanly. Now switch to Unity and let it recompile."
